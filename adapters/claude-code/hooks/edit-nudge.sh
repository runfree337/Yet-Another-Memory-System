...
# repo root and the session-scoped once-per-target marker, then hands the hook
# JSON to memory-graph.py, which re-validates `tool_input.file_path`, checks
# containment against the derived graph, self-suppresses on a memory-channel
# target, and stays silent on an uncovered file. Silent no-op without
# memory-graph.py; never blocks — always exit 0.
#
