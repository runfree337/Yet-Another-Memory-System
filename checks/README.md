...
- **`backlog-check.py`** — intégrité du `backlog/` (modèle frontmatter) : chaque chantier doc-backed = un dossier `<id>/` dont l'`ETAT.md` porte un frontmatter complet (`id/titre/statut/jalon/apres/docs/maj`) ; `id`=dossier, `statut`⟺rubriques, `jalon`⟺groupe INDEX, `apres`→id réel, `docs`⟺compagnons ; pas d'orphelin ni de fichier plat de chantier ; pointeurs résolus ; pas de case Markdown dans l'INDEX. Vues `--board` (chantiers par jalon + comptes) et `--state <id>` (un chantier déroulé), `--json` possible. **`--stamp --staged`** : pose `maj = aujourd'hui` sur les ETAT.md indexés + re-stage — **à câbler au PRÉ-COMMIT** (hook git `pre-commit`, ou l'équivalent de ton outil) pour que `maj` reste juste sans effort.
- **`feature-map-check.py`** — structure de `FEATURE_MAP.md` : chaque fiche porte ses clés-cœur (une ligne `**Rôle`, ≥ 1 chemin de code, ≥ 1 réf durable — clé `**Doc` durable ou id `D-*`), Sommaire↔fiches concordent (ancres GitHub), aucune réf transitoire (`backlog/`), alerte granularité **soft**. Dead-path délégué à `doc-refs-check.py`. Patterns durable/transitoire/code à adapter au layout du projet.
- **`decisions-check.py`** — intégrité des `decisions/` : concordance fichiers `D-*.md` ↔ lignes d'`INDEX.md`, par identifiant.
- **`memory-check.py`** — intégrité du canal **Mémoire** : un fait par fichier + frontmatter (`../memory/<slug>.md`), `../MEMORY.md` = index. Frontmatter sans `confiance:` sur une source externe → bloquant ; `confiance: à vérifier` → candidate à l'étage 2 ; fichier↔index qui ne concordent pas (orphelin, lien mort) → bloquant. Suit `GABARIT.md` à la lettre.
