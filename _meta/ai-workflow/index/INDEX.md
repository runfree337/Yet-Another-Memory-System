...
`index/manifest.tsv` (`chemin<TAB>intent`, une ligne par fichier, **généré**, pas maintenu à la
main).

- **Écriture** — `index/manifest.py` (`set` / `rm` / `get` / `stamp`) : seul moyen d'éditer le
  manifeste. Ne code aucune extension ni racine en dur — il lit `index/index-config.json`, comme
...
**Le projet définit ce qu'il indexe** — racines + extensions — dans `index/index-config.json`
